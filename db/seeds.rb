...
# Reviews
re1 = Review.create(rating: 2, comment: 'It is what it is 🤷‍♂️', user_id: u2.id, restaurant_id: r1.id)
re2 = Review.create(rating: 5, comment: 'So delicious! Perfect for any carnivore~', user_id: u1.id, restaurant_id: r2.id)
re3 = Review.create(rating: 4, comment: 'Great soft-serve, will come back.', user_id: u1.id, restaurant_id: r5.id)
re4 = Review.create(rating: 3, comment: 'Delicious, wish it had more options', user_id: u3.id, restaurant_id: r5.id)
re5 = Review.create(rating: 4, comment: 'Great Korean food!', user_id: u3.id, restaurant_id: r3.id)
